...
def on_mqtt_message(client, userdata, msg):
    ...
    if topic == TOPIC_ACK:
        # Podrías parsear ACK y mostrar en UI
        return
...
def start_mqtt_if_needed():
    ...
    try:
        ...
        st.sidebar.error(f"Error conectando a MQTT: {e}")
        return
...
with st.sidebar:
    ...
    auto_refresh = st.checkbox("Auto-refrescar tiempo real", value=True,
    ...
    filter_metric_rt = st.multiselect("Métricas en tiempo real",
                                      metric_options,
...
st.markdown('<div class="subtitle">Monitoreo de calidad de aire, temperatura, humedad y control de actuadores en tiempo casi real.</div>', unsafe_allow_html=True)
st.markdown("---")
...
with col_cards:
    ...
    if not df_tail.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.metric("🌡️ Temp (°C)", f"{last.get('value'):.1f}" if last["metric"] == "temp_c" else "-")
        with c2:
            ...
            st.metric("💧 Humedad (%)", f"{hum_val:.1f}" if hum_val is not None else "-")
...
if not sb_client:
    st.warning("Supabase no está configurado (falta URL o ANON KEY).")
else:
    try:
